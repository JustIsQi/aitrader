...
#from alpha.logger import logger
...
class AlphaDataset:
    ...
    def prepare_data(self,df, filters: dict | None = None, max_workers: int | None = None) -> None:
        """
        ...
        args: list[tuple] = [(df, name, expression) for name, expression in expressions]
        for item in args:
            result = calculate_by_expression(df,item[2])["data"].alias(item[1])
        ...
        self.result_df = df.with_columns(results)
        ...
        for name, result in tqdm(self.feature_results.items()):
            ...
            self.result_df = self.result_df.join(result, on=["date", "symbol"], how="inner")
            ...
            for vt_symbol, ranges in tqdm(filters.items(), total=len(filters)):
                for start, end in ranges:
                    temp_df = raw_df.filter(
                        (pl.col("symbol") == vt_symbol) & (pl.col("date") >= pl.lit(start)) & (pl.col("date") <= pl.lit(end))
                    )
        ...
        select_columns: list[str] = ["date", "symbol"] + raw_df.columns[df.width:]
        #self.raw_df = raw_df.select(select_columns).sort(["date", "symbol"])
        self.raw_df = raw_df.sort(["date", "symbol"])
