...
def ts_min(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_min(window, min_samples=1).over("symbol")
    )
    return DataProxy(df)
...
def ts_max(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_max(window, min_samples=1).over("symbol")
    )
    return DataProxy(df)
...
def ts_argmax(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_map(lambda s: cast(int, s.arg_max()) + 1, window).over("symbol")
    )
    return DataProxy(df)
...
def ts_argmin(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_map(lambda s: cast(int, s.arg_min()) + 1, window).over("symbol")
    )
    return DataProxy(df)
...
def ts_rank(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_map(lambda s: stats.percentileofscore(s, s[-1]) / 100, window).over("symbol")
    )
    return DataProxy(df)
...
def ts_sum(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_sum(window).over("symbol")
    )
    return DataProxy(df)
...
def ts_mean(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").cast(pl.Float32).rolling_map(lambda s: np.nanmean(s), window, min_samples=1).over("symbol")
    )
    return DataProxy(df)
...
def ts_std(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_map(lambda s: np.nanstd(s, ddof=0), window, min_samples=1).over("symbol")
    )
    return DataProxy(df)
...
def ts_slope(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_map(lambda s: np.polyfit(np.arange(len(s)), s, 1)[0], window).over("symbol")
    )
    return DataProxy(df)
...
def ts_quantile(feature: DataProxy, window: int, quantile: float) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_map(lambda s: s.quantile(quantile=quantile, interpolation="linear"), window).over("symbol")
    )
    return DataProxy(df)
...
def ts_rsquare(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_map(lambda s: rsquare(s), window).over("symbol"))
    return DataProxy(df)
...
def ts_resi(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.col("data").rolling_map(lambda s: resi(s), window).over("symbol")
    )
    return DataProxy(df)
...
def ts_corr(feature1: DataProxy, feature2: DataProxy, window: int) -> DataProxy:
    ...
    df_merged: pl.DataFrame = feature1.df.join(feature2.df, on=["date", "symbol"])

    df: pl.DataFrame = df_merged.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.rolling_corr("data", "data_right", window_size=window, min_samples=1).over("symbol").alias("data")
    )
...
def ts_less(feature1: DataProxy, feature2: DataProxy | float) -> DataProxy:
    ...
    df: pl.DataFrame = df_merged.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.min_horizontal("data", "data_right").over("symbol").alias("data")
    )
...
def ts_greater(feature1: DataProxy, feature2: DataProxy | float) -> DataProxy:
    ...
    df: pl.DataFrame = df_merged.select(
        pl.col("date"),
        pl.col("symbol"),
        pl.max_horizontal("data", "data_right").over("symbol").alias("data")
    )
