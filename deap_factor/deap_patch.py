...
from utils import stringify_for_sympy
    ...
    *ngen* generations are done, the algorithm returns a tuple with the final
    ...
    for gen in range(1, ngen + 1):
        ...
        for p in population:
            ...
            dict_results[expr] = p.fitness.values[0]

        sorted_dict = dict(sorted(dict_results.items(), key=lambda item: item[1], reverse=True))
        print(sorted_dict)
        record = stats.compile(population) if stats is not None else {}
        print(record)
