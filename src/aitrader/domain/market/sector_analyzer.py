...
功能:
1. 从本地数据库读取板块与行情数据（Wind 数据落库后读取）
2. 计算技术指标(MA5/10, RSI)
3. 统计涨停家数、连板家数
4. 综合评分排序
...
class SectorAnalyzer:
    ...
    def fetch_sector_data(self, date: str) -> pd.DataFrame:
        ...
        try:
            ...
            df = pd.DataFrame({
                ...
                # Wind 行情表中没有板块资金流字段，这里置 0，后续靠量价/涨停等打分
                "main_net_inflow": [0.0] * len(sector_names),
            })
            logger.info(f"✓ 从数据库加载 {len(df)} 个板块")
            return df
    ...
    def calculate_sector_scores(self, date: str) -> List[SectorScore]:
        ...
        # 2. 如果数据库没有聚合板块数据，则按股票维度实时计算（fallback）
            ...
            try:
                ...
                if not df_sector.empty and 'sector_name' in df_sector.columns:
                    ...
                    if not sector_row.empty:
                        ...
                        # Wind 路径下 main_net_inflow 目前使用本地占位值，不做额外换算
                    else:
